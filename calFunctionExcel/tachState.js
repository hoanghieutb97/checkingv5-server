...
async function tachState(items, cardId) {
        ...
        if (status) { // xoa cac file oke 
            const createExcelFiles = async () => {
                ...
                for (let i = 0; i < allStates.length; i++) {
                    ...
                    const filePath = path.join(tachStatePath, nameTong + '.xlsx');
                    ...
                    ListFIleTach.push(nameTong);
                }
                // Chờ cho đến khi tất cả Promises trong mảng hoàn thành
                Promise.all(excelCreationPromises).then(() => {
                    moveToListArchive(cardId);
            ...
            createExcelFiles(); // Gọi hàm để thực hiện
